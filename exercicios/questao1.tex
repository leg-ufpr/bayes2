...
%\usepackage{Sweave}
\title{CE-315: Inferência Bayesiana -- $1^a$ Prova (17/10/2024)}
...
\item Modela-se com a distribuição binomial negativa o número de entrevistas necessárias para se conseguir três candidatos aprovados em uma fase inicial de um processo seletivo. Considera-se uma abordagem Bayesiana. Não se tem ideia da probabilidade de aprovação e decide-se por uma distribuição {\it a priori} que reflita tal situação. Em uma rodada de seleção foram necessárias 20 entrevistas para se conseguir os três aprovados. 
  \begin{enumerate}
  \item Identifique o modelo, a variável observada e o parâmetro do modelo.
  \item Escreva a função de densidade de probabilidades para o parâmetro do modelo.
  ...
  \item Obter a expressão da distribuição marginal (preditiva a priori).
  \item Explique como obter simulações dos valores desta distribuição marginal.
  \item Obtenha a expressão da posteriori e identifique qual a distribuição de probabilidades.
  ...
  \item Obtenha a expressão da distribuição preditiva para uma nova observação após obter esta primeira amostra?
  \item Como pode-se obter o valor esperado da preditiva do item anterior?
