...
- **25/03 (Qua)**: Inferência no modelo normal e introdução à regressão bayesiana.
  ...
  - **Atividades:** Ver no final da página acima. Usar os [dados de altura e peso](dados/dados_turma_bayes2.csv) da turma para desenvolver o modelo e
    pensar em diferentes prioris.
- **30/03 (Seg)**: Introdução à atualização bayesiana em regressão. Análise dos
  dados de altura e peso da turma (ver [script](scripts/turma.R)). Finalização
  dos [modelos de regressão](pages/regressao_linear_unificado_aula.html).
- **01/04 (Qua)** Prova 1.
- **06/04 (Seg)** "O falso dilema: Estatística Bayesiana vs Clássica"
  [PDF](slides/pos.pdf)
- **08/04 (Qua)** Atualização sequencial. Exemplo
  [Beta-Binomial](scripts/atualizacao-sequencial-beta-binomial.R).
- **13/04 (Seg)** Demonstração da obtenção das posteriores para parâmetros de
  regressão com prioris normais.
- **15/04 (Qua)** [Regressão múltipla](pages/cap7_modelos_lineares_anova_ancova.html).
